...
Far-right parties indicate active agenda-setting behavior by introducing migration-speech into debates. %, particularly about economy and security. 
For other party blocs, this phenomenon is too rare to assert a defnite pattern and can partly be drawn to LDA misclassifications. %However, there are indications of a polar divide: Green and left parties tend to introduce migration to debates on humanitarian issues. Furhter research can examine whether such a left-right distinction shapes where migration discourse is fed into the debate.

Dimensionality reduction reveals two largely interpretable axes separating parties in semantic space. The first axis is clearly interpretable, while the second is less distinct and characterized by extreme examples in positive direction referencing security concerns in Eastern Europe. Although a definitive interpretation of the underlying political dimensions requires substantial domain knowledge, word associations and extreme speeches along each axis permit tentative conclusions. Despite the noisy nature of general-purpose embeddings, the strong party separation along the axes supports cautious interpretation. We interpret the first axis as \verb|conciliatory| $\Leftrightarrow$ \verb|oppositional|. The second axis is less distinct but could reflect a \verb|moral| $\Leftrightarrow$ \verb|pragmatic-benefits| / \verb|security| dimension. Overall, our results indicate that PLS can be useful for uncovering latent political dimensions in high-dimensional embeddings.
