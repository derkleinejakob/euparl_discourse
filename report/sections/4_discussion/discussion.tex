...
Our analysis of European Parliament speeches (2014 to 2024) reveal systematic differences in migration rhetoric between party groups. 

% LDA & Agenda setting  
% TODO: talk about LDA misclassification rate as limitations? 

Far-right parties indicate active agenda-setting behavior by introducing migration into debates, particularly about economy and security. For other party blocks, this phenomenon is too rare to assert a defnite pattern and can partly be drawn to misclassifications of the LDA model. However, there are indications of a polar divide: Green and left parties tend to introduce migration to debates on humanitarian issues. Examining whether such a left-right distinction shapes where migration discourse is fed into the debate is a promising direction for further research.

Dimensionality reduction via PLS reveals two largely interpretable axes that separate parties in semantic space. While the first axis is clearly interpretable, the positive direction of the second includes terms related to Eastern Europe, raising questions about its interpretability and meaningfulness. Although a definitive interpretation of the underlying political dimensions requires substantial domain knowledge, word associations and extreme speeches along each axis permit tentative conclusions. Despite the noisy nature of general-purpose embeddings, which likely contributes to blurred axis labels, the strong party separation along the axes supports cautious interpretation. We interpret the first PLS axis as a
\verb|conciliatory| $\Leftrightarrow$ \verb|oppositional| discourse spectrum. The second axis is less distinct but likely reflects a \verb|moral| $\Leftrightarrow$ \verb|pragmatic-benefits| / \verb|security| dimension. Overall, these results indicate that PLS is a useful exploratory method for uncovering latent dimensions of political discourse in high-dimensional text embeddings.

todo: what can we see? extreme-rights are different in first dimension, left and green especially in second dimension. 
...
Across all analyses, right-wing populist speech emerges as distinct from other parties. In particular, parties on the political right appear more likely to foreground migration-related themes, while placing less emphasis on humanitarian concerns and framing immigrants in terms of threat, harm, and illegality.
